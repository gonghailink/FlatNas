...
<template>
  <div v-if="show" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all scale-100 border border-gray-100">
      
      <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
        <h3 class="font-bold text-gray-800">{{ title || '请输入密码确认操作' }}</h3>
        <button @click="close" class="text-gray-400 hover:text-gray-600 leading-none text-xl">&times;</button>
      </div>
